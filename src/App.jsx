import { askHuggingFace } from "./huggingface";
...
export default function App() {
    ...
    const handleChat = async () => {
        ...
        Here is a database of exams:
        ${JSON.stringify(allExams, null, 2)}

        INSTRUCTIONS:
        - If the question is about exams → use the database FIRST
        - Recommend relevant exams
        - If needed, add general knowledge
        - Be clear, structured, and helpful
        - Answer like a real advisor (not robotic)
        `;
            ...
            try {
            reply = await fetchExamData(prompt);

            if (
                !reply ||
                reply.toLowerCase().includes("unavailable") ||
                reply.toLowerCase().includes("busy") ||
                reply.toLowerCase().includes("high demand")
            ) {
                throw new Error("Gemini failed");
            }

            } catch (err) {
            console.log("Switching to Hugging Face...");

            reply = await askHuggingFace(prompt);
            }
           ...
           setMessages(prev => [
            ...prev,
            { role: "ai", text: cleanText, model: modelUsed }
            ]);

        } catch (err) {
            console.error(err);

            // ✅ SMART FALLBACK (not dumb text)
            setMessages(prev => [
                ...prev,
                {
                    role: "ai",
                    text: "I'm having trouble right now, but generally you can explore exams like JEE, NEET, CUET depending on your interest. Try again in a moment."
                }
            ]);
        }
...
Here are available exams:
${JSON.stringify(allExams, null, 2)}

Suggest the best options and explain briefly.
`;

            const result = await fetchExamData(prompt); // reuse Gemini
                ...
                {tab === "browse" && (
                    <>
                        ...
                        <div style={{
                            ...
                            <h3 style={{ marginBottom: "10px" }}>AI Assistant</h3>
                                    ...
                                    >
                                        ...
                                        {messages.length === 0 && !loadingChat && (
                                            <div style={{ color: "#888", fontSize: "14px" }}>
                                                Ask something like "best engineering exams"
                                            </div>
                                        )}
                                        ...
                                        <span style={{
                                            ...
                                            padding: "8px 12px",
                                            borderRadius: "12px",
                                            background: msg.role === "user" ? "#e8622a" : "#f1f1f1",
                            ...
                            {/* ✅ INPUT OUTSIDE MESSAGE BOX */}
                            <div style={{ display: "flex", gap: "8px" }}>
                                <input
                                    value={chatInput}
                                    onChange={(e) => setChatInput(e.target.value)}
                                    placeholder="Ask something like 'best engineering exams'"
                                    style={{
                                        flex: 1,
                                        padding: "10px",
                                        borderRadius: "10px",
                        ...
                        {aiAnswer && (
                            <div style={{
                                ...
                                <p style={{ marginTop: "8px" }}>
                                    {loadingAskAI ? "Thinking..." : aiAnswer}
                                </p>
                            </div>
                        )}
